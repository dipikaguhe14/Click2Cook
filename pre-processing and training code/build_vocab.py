...
def update_counter(list_, counter_toks, istrain=False):
    for sentence in list_:
        tokens = nltk.tokenize.word_tokenize(sentence)#create a hah map key word and value will be the count of that word in sentence
        if istrain:
...
def build_vocab_recipe1m(args):
    ...
    for i, entry in enumerate(layer2):
        id2im[entry['id']] = i # giving array proper index to layer2.json id
    ...
    for i, entry in enumerate(dets):
        idx2ind[entry['id']] = i # giving array proper index to det_ingrs.json id
    ...
    if os.path.exists(ingrs_file) and os.path.exists(instrs_file) and not args.forcegen:
        ...
        counter_toks = Counter() # hashmap count of words
        counter_ingrs = Counter()
        counter_ingrs_raw = Counter()

        for i, entry in tqdm(enumerate(layer1)): # tqdm mske iteration fast
            ...
            for instr in instrs:
                instr = instr['text']
                instr = get_instruction(instr, replace_dict_instrs) #filtering instructions just as ingredients words
                if len(instr) > 0:
    ...
    for i, entry in tqdm(enumerate(layer1)):
        ...
        if entry['id'] in id2im.keys(): #if given id is present in layer2.json
