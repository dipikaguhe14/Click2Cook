...
class Recipe1MDataset(data.Dataset):
    ...
    def __init__(self, data_dir, aux_data_dir, split, maxseqlen, maxnuminstrs, maxnumlabels, maxnumims,
        ...
        self.ingrs_vocab = pickle.load(open(os.path.join(aux_data_dir, suff + 'recipe1m_vocab_ingrs.pkl'), 'rb')) #contain ingredients words
        self.instrs_vocab = pickle.load(open(os.path.join(aux_data_dir, suff + 'recipe1m_vocab_toks.pkl'), 'rb')) #contains instrucions
        self.dataset = pickle.load(open(os.path.join(aux_data_dir, suff + 'recipe1m_'+split+'.pkl'), 'rb')) #contains full datastet title,ingredients,instructions,images

        self.label2word = self.get_ingrs_vocab() #ingredient vocab words
    ...
    def __getitem__(self, index):
        ...
        img_id = sample['id']
        captions = sample['tokenized'] #instructions
        ...
        tokens.append('<eoi>')
        for c in captions:
            tokens.extend(c) #adding instructions words into tokens
            tokens.append('<eoi>')
        ...
        true_ingr_idxs = []
        for i in range(len(labels)): #ingredients
        ...
        if len(paths) == 0:
            ...
            if self.split == 'train':
                for(j in range(0,len(paths))):
                    path=paths[j]
                    if(os.path.exist(os.path.join(self.root, path[0], path[1], path[2], path[3], path))):
                        img_idx=j
                        break
